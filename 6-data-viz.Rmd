...
*and far, far, far suprior to ten numbers!
...
```{r point, fig.width=8, fig.height = 6}
starwars %>% 
...
```{r lm-drop-outlier, fig.width=8, fig.height = 6}
...
```{r lm-drop-outlier2, fig.width=8, fig.height = 6}
...
```{r lm-by-group, fig.width=8, fig.height = 6}
starwars %>%
...
```{r starwars-height, fig.width=10, fig.height= 5.5}
starwars %>% unnest(films) %>% filter(films %in% c("Return of the Jedi", "The Force Awakens")) %>% 
ggplot( aes(x = birth_year, y = height, size = mass, color = species) ) + 
  ...
  labs(x = "Age", y = "Height (cm)", color = "Species", size = "Mass (kg)")
...
```{r map-world, fig.width=8, fig.height=4}
# library(maptools)
# library(maps)
...
ggplot() +  
  ...
  # A point for madison 43.0731° N, 89.4012° W
  geom_point(aes(x=-89.4012, y=43.0731), color = "Red") +
  geom_text(aes(x=-89.4012, y=43.0731) , label = "Madison", size=3) 
```
...
```{r map-wi, fig.width=8, fig.height=4}
wi <- map_data("county", "wisconsin")
mid_range <- function(x) mean(range(x))
seats <- plyr::ddply(wi, "subregion", plyr::colwise(mid_range, c("lat", "long")))
ggplot(wi, aes(long, lat)) +
