---
title: "Tidy text analysis example:"
subtitle: "U.S. Legislators' correspondence with government agencies"
author: 
output:
    html_document:
    toc: true
...
stopwords <- str_c(
  "[0-9]| [a-z] |^[a-z] | [a-z]$| ", # numbers or single letters
  str_c(stop_words$word, collapse = " | "), # with a space on either side
  # str_c(stop_words$word, collapse = "$| "), # at the end of a string
  # str_c(stop_words$word, collapse = " |^"), # at the start of a string
  collapse = " | " )
# Add to the list of things to exclude
stopwords <- str_c(stopwords, " |senator. |representative. |write. |writing| submit. |submitting|submitted|letter.|signature.")
...
```{r single-words, fig.width=10, fig.height=4}
words <- d %>% 
...
We can also look at the frequency of **pairs of words** ("bi-grams"). First, we'll look at common bigrams, filtering out stop words again (since we don't want pairs like "of the" and "in the"):

```{r bigrams-all, fig.width=10, fig.height=4}
bigrams <- d %>% 
  group_by(Department) %>% 
 ...
 # Split the bigram column into two columns
  ...
  unite(bigram, word1, word2, sep = " ") %>% # (can also use glue::glue_data)
...
We can replicate the ["She Giggles, He Gallops"](https://pudding.cool/2017/08/screen-direction/) idea, looking for gendered verbs by counting the bigrams that match "he X" and "she X".
...
```{r sentiment-bing, message=FALSE}
sentiment <- d %>% 
...
- We can also capture the number of punctuation marks, hashtags, mentions, etc. using the `textfeatures()` function:
...
Recent scholarship has begun to use mixed-member models to describe political relationships using the relative emphasis of different topics in texts as the dependent variable. 
