...
- `anti_join(stop_words)` removes all the common stop words like "a" and "the". You can do the same thing with `filter()` like so: `filter(!(word %in% stop_words$word))` 
- `count()` how many times each word appears and sort the list (`count(word)` is like `group_by(word) %>% summarize(n = n()) %>% ungroup()` )
...
Words are only one of many features of a text. 
